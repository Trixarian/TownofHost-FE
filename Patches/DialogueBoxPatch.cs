...
internal class DialogueBoxPatch
{
    ...
    {
        if (!PlayerControl.LocalPlayer.inVent && dialouge.Contains("<size=0%>tohfe</size>") && GameStates.IsInTask)
        {
    ...
    /*
     * Lets add a <size=0%>tohfe</size> at where you will use the dialogue box.
     */
}
